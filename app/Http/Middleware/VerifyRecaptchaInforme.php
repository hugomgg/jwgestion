<?php

namespace App\Http\Middleware;

use Closure;
use Illuminate\Http\Request;
use Symfony\Component\HttpFoundation\Response;
use ReCaptcha\ReCaptcha;
use Illuminate\Support\Facades\Log;

class VerifyRecaptchaInforme
{
    /**
     * Handle an incoming request.
     *
     * @param  \Closure(\Illuminate\Http\Request): (\Symfony\Component\HttpFoundation\Response)  $next
     */
    public function handle(Request $request, Closure $next): Response
    {
        // Si reCAPTCHA está deshabilitado, continuar sin validación
        if (!config('recaptcha.enabled')) {
            return $next($request);
        }

        // Obtener el token de reCAPTCHA del request
        $recaptchaToken = $request->input('g-recaptcha-response');

        // Si no hay token, rechazar con respuesta JSON
        if (!$recaptchaToken) {
            return response()->json([
                'success' => false,
                'message' => 'Por favor, complete la verificación de seguridad.',
                'errors' => [
                    'recaptcha' => ['Por favor, complete la verificación de seguridad.']
                ]
            ], 422);
        }

        try {
            // Crear instancia de reCAPTCHA con la clave secreta
            $recaptcha = new ReCaptcha(config('recaptcha.secret_key'));
            
            // Verificar el token con la IP del usuario
            $response = $recaptcha->setExpectedAction('informe_submit')
                                  ->setScoreThreshold(config('recaptcha.score_threshold'))
                                  ->verify($recaptchaToken, $request->ip());

            // Si la verificación falla
            if (!$response->isSuccess()) {
                Log::warning('reCAPTCHA verification failed (informe)', [
                    'ip' => $request->ip(),
                    'errors' => $response->getErrorCodes(),
                    'score' => $response->getScore()
                ]);

                return response()->json([
                    'success' => false,
                    'message' => 'Verificación de seguridad fallida. Por favor, intente nuevamente.',
                    'errors' => [
                        'recaptcha' => ['Verificación de seguridad fallida. Por favor, intente nuevamente.']
                    ]
                ], 422);
            }

            // Verificar el score (solo para reCAPTCHA v3)
            $score = $response->getScore();
            if ($score < config('recaptcha.score_threshold')) {
                Log::warning('reCAPTCHA score too low (informe)', [
                    'ip' => $request->ip(),
                    'score' => $score,
                    'threshold' => config('recaptcha.score_threshold')
                ]);

                return response()->json([
                    'success' => false,
                    'message' => 'Su actividad parece sospechosa. Por favor, intente más tarde.',
                    'errors' => [
                        'recaptcha' => ['Su actividad parece sospechosa. Por favor, intente más tarde.']
                    ]
                ], 422);
            }

            // Log exitoso
            Log::info('reCAPTCHA verification successful (informe)', [
                'ip' => $request->ip(),
                'score' => $score
            ]);

        } catch (\Exception $e) {
            Log::error('reCAPTCHA verification error (informe)', [
                'error' => $e->getMessage(),
                'ip' => $request->ip()
            ]);

            // En caso de error del servicio, rechazar con error
            return response()->json([
                'success' => false,
                'message' => 'Error en la verificación de seguridad. Por favor, intente nuevamente.',
                'errors' => [
                    'recaptcha' => ['Error en la verificación de seguridad. Por favor, intente nuevamente.']
                ]
            ], 422);
        }

        return $next($request);
    }
}
